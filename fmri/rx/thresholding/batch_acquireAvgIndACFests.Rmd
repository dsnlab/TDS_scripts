...
df.summary= as.data.frame(cbind(rx_models))
df.summary[, c("mean1","mean2","mean3")]= NA

for (i in 1:length(rx_models)){
  setwd(paste0(rx_directory,rx_models[i]))
  ...
  test=summarise(df.logs, mean1=mean(df.logs.acf$V1), mean2=mean(df.logs.acf$V2), mean3=mean(df.logs.acf$V3))
  
  df.summary[i, c("mean1","mean2","mean3")]= c(test$mean1,test$mean2, test$mean3) #summarise results: mean
  #mean4=mean(df.logs.acf$V4)) 
...
df.summary_long= gather(df.summary, `mean1`, `mean2`, 'mean3', key = "position", value = "values")
df.summary_long= df.summary_long[order(df.summary_long$rx_models),] #order by models

setwd(rx_directory)
df.summary_long$rx_models= as.character(df.summary_long$rx_models)
write(df.summary_long, "batch_indACFest_summary.csv")
write(df.summary_long$rx_models, "ordered_rxmodels.txt")
write(df.summary_long$values, "indACFest_orderedmeans.txt")
