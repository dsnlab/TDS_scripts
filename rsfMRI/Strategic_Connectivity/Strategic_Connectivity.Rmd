...
Time by run index effects
```{r}
load(file= paste0("/Volumes/TDS/bids_data/derivatives/strategic_connectivity/",
                    "connectivity_models/strategy_models_DT.RDS"))
...
Testing ground
```{r}
test_analysis <- strategy_modelsDT %>%
...
unique(test_analysis$seed)
unique(test_analysis$target)
...
## Make Heat Map
gordon_key<-read.csv("./gordon_subcortical_parcel_key.csv",header = TRUE,stringsAsFactors = F)
test_analysis_heatmap<-test_analysis %>%
  select(seed, target, chisq) %>%
  mutate(chisq=as.numeric(chisq))
test_analysis_heatmap <- left_join(x = test_analysis_heatmap,y=(gordon_key %>% select(parcel_name,heatmap_name) %>%
                                                                  mutate(seed=parcel_name)),
                                  by=c("seed")) %>%
  mutate(seed_heat=heatmap_name) %>%
  select(-heatmap_name,-parcel_name)
test_analysis_heatmap <- left_join(x = test_analysis_heatmap,y=(gordon_key %>% select(parcel_name,heatmap_name) %>%
                                                                  mutate(target=parcel_name)),
                                  by=c("target")) %>%
  mutate(target_heat=heatmap_name) %>%
  select(-heatmap_name,-parcel_name) %>%
  arrange(seed_heat,target_heat)

heatmapdf <- expand.grid(gordon_key$heatmap_name,gordon_key$heatmap_name)
heatmapdf<- heatmapdf %>% mutate(chisq=0)
heatmapdf$Var1=0
heatmapdf$Var2=0
makeheatmap=function(i){
  row<-which(grepl(test_analysis_heatmap$seed_heat[i], heatmapdf$Var1)&
               grepl(test_analysis_heatmap$target_heat[i], heatmapdf$Var2))
  heatmapdf$chisq[row]=test_analysis_heatmap$chisq[i]
  row2<-which(grepl(test_analysis_heatmap$target_heat[i], heatmapdf$Var1)&
                      grepl(test_analysis_heatmap$seed_heat[i], heatmapdf$Var2))
  heatmapdf$chisq[row2]=test_analysis_heatmap$chisq[i]}
mclapply(X=(seq(1:59340)),
         FUN=makeheatmap,
         mc.cores = numcores)
View(heatmapdf)
...
# Plot the heat map
ggplot(data = test_analysis_heatmap, aes(x=seed_heat, y=target_heat, fill=chisq)) + 
  geom_raster()
  geom_tile()

m<-ggplot(data = test_analysis_heatmap, aes(x=seed_heat, y=target_heat, fill=chisq)) +
  geom_tile() 
m + scale_fill_distiller(palette = )
