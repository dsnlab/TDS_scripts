...
## Make Heat Map
gordon_key<-read.csv("./gordon_parcel_key.csv",header = TRUE,stringsAsFactors = F)
test_analysis_heatmap<-test_analysis %>%
  select(seed, target, chisq)
heatmapdf <- expand.grid(gordon_key$parcel_name,gordon_subcortical$parcel_name)
heatmapdf<- heatmapdf %>% mutate(chisq=0)
heatmapdf$Var1b=0
heatmapdf$Var2b=0
for (i in 1:length(test_analysis$seed)){
  row<-which(grepl(test_analysis$seed[i], heatmapdf$Var1) & grepl(test_analysis$target[i], heatmapdf$Var2))
  heatmapdf$chisq[row]=as.numeric(test_analysis$chisq[i])
  row2<-which(grepl(test_analysis$target[i], heatmapdf$Var1) & grepl(test_analysis$seed[i], heatmapdf$Var2))
  heatmapdf$chisq[row2]=as.numeric(test_analysis$chisq[i])}
View(heatmapdf)

# Plot the heat map
m<-ggplot(data = heatmapdf, aes(x=Var1, y=Var2, fill=chisq)) +
  geom_tile() 
