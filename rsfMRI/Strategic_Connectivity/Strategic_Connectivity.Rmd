...
Exclude subs with resting state issues
```{r}
sublist<- sublist %>%
...
ylg_timing<-ylg_filtered %>%
  ...
  select(-subject.name) %>%
  filter(!group=="jj") %>%
  mutate(run_indexnum=as.numeric(run_index))
...
## Find best fitting polynomial model for full dataset
nullmodel=lme(time ~ 1, method="ML", random = ~1|sub, data=ylg_timing)
linmodel=lme(time ~ run_indexnum, method="ML", random = ~1|sub, data=ylg_timing)
groupmodel=lme(time ~ group, method="ML", random = ~1|sub, data=ylg_timing)
lingroupmainmodel=lme(time ~ run_indexnum+group, method="ML", random = ~1|sub, data=ylg_timing)
lingroupintmodel=lme(time ~ run_indexnum*group, method="ML", random = ~1|sub, data=ylg_timing)
...
commmodel=lme(time ~ as.numeric(run_index), method="ML", random = ~1|sub, data=(ylg_timing %>%
                                                                    filter(group=="community")))
anova(commnullmodel,commmodel)

fosternullmodel=lme(time ~ 1, method="ML", random = ~1|sub, data=(ylg_timing %>%
                                                                    filter(group=="foster")))
fostermodel=lme(time ~ as.numeric(run_index), method="ML", random = ~1|sub, data=(ylg_timing %>%
                                                                    filter(group=="foster")))
...
## Graph it yo!
group.colors <- c(community = "forestgreen", foster = "dodgerblue")

ylg_timing_graph<-ggplot(ylg_timing,
                         aes(y=time,
                             x=as.numeric(run_index)))+
  geom_line(data=comm.data.pred,
              aes(x=run_index, y=time), size=.7, colour="forestgreen")+
  geom_ribbon(data=comm.data.pred,
              aes(ymin=y.lower, ymax=y.upper), alpha=0.2, fill="forestgreen")+
  geom_line(data=foster.data.pred,
              aes(x=run_index, y=time), size=.7, colour="dodgerblue")+
  geom_ribbon(data=foster.data.pred,
              aes(ymin=y.lower, ymax=y.upper), alpha=0.2, fill="dodgerblue")+
  geom_line(data=ylg_timing,
            aes(colour=group, group=sub),size=.3,alpha=0.3)+
  scale_colour_manual(values = c("forestgreen", "dodgerblue"))+
  geom_point(data=ylg_timing,
             aes(colour=group, group=sub),size=2,alpha=0.3)+
  ylab("Time to complete run") +
  xlab("Run index")+
  theme_bw() +
...
## Check out if condition impacts time
condition_model=lme(time ~ condition, method="ML", random = ~1|sub, data=ylg_timing)
condition_runindex_model=lme(time ~ condition+run_indexnum, method="ML", random = ~1|sub, data=ylg_timing)
condition_intrunindex_model=lme(time ~ condition*run_indexnum, method="ML", random = ~1|sub, data=ylg_timing)
condition_intrunindex_group_model=lme(time ~ condition*run_indexnum+group, method="ML", random = ~1|sub, data=ylg_timing)
condition_intrunindex_intgroup_model=lme(time ~ condition*run_indexnum*group, method="ML", random = ~1|sub, data=ylg_timing)
...
if(!file.exists("/Volumes/TDS/bids_data/derivatives/strategic_connectivity/socbrain_coisDT.RDS")){
  ...
  save(social_coisDT,file = "/Volumes/TDS/bids_data/derivatives/strategic_connectivity/social_coisDT.RDS")
} else {
  load(file = "/Volumes/TDS/bids_data/derivatives/strategic_connectivity/social_coisDT.RDS")
}
...
if(rerun_models){
    ...
    gordon_models<-mclapply(X=as.character(as.list(gordon_cois$coi)), 
      ...
      FUN=function(coi_name, demodata, coidat){
        adf<-merge(as.data.table(filter(coidat,coi==coi_name)),
                   ...
                   allow.cartesian=T) %>%
        filter(!group=="juvjus")
        beh_mod=lme(time ~ condition*run_indexnum,
                      method="ML",
                      random = ~1|sub,
                      data=adf)
        group_mod=lme(time ~ condition*run_indexnum+group,
                      method="ML",
                      random = ~1|sub,
                      data=adf)
        coi_mod=lme(time ~ condition*run_indexnum+cor,
                         method="ML",
                         random = ~1|sub,
                         data=adf)
        mod_comp<-anova(beh_mod,coi_mod)
        behvsgroup<-anova(beh_mod,group_mod)
  ...
  print(object.size(gordon_modelsDT), units='Mb')
  rm(gordon_models);gc()
  save(gordon_modelsDT, file= paste0("/Volumes/TDS/bids_data/derivatives/strategic_connectivity/gordoncois_strategymodels_DT.RDS"))
} else {
  load(file= paste0("/Volumes/TDS/bids_data/derivatives/strategic_connectivity/gordoncois_strategymodels_DT.RDS"))
}
