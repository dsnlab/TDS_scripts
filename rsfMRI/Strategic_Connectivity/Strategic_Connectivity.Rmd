...
if(resave_plots){gender_age_group_density<-ggplot(hist_by_age_gender,
...
if(resave_plots){box_age_group<-ggplot(qualtrics_data,
       aes(x=group,
           y=s2_age,
           fill=group)) +
...
if(resave_plots){box_pds_group<-ggplot(qualtrics_data,
       aes(x=group,
           y=PDS_mean_score,
           fill=group)) +
  geom_boxplot(aes(fill=group)) + 
...
if(resave_plots){box_iq_group<-ggplot(qualtrics_data,
       aes(x=group,
           y=fsiq2,
           fill=group)) +
  geom_boxplot(aes(fill=group)) + 
...
if(resave_plots){box_adversity_group<-ggplot(qualtrics_data,
       aes(x=group,
           y=ace_total,
           fill=group)) +
  geom_boxplot(aes(fill=group)) + 
...
if(resave_plots){box_maltreatment_group<-ggplot(qualtrics_data,
       aes(x=group,
           y=ace_maltreatment,
           fill=group)) +
  geom_boxplot(aes(fill=group)) + 
...
if(resave_plots){model_time_by_runindex<-ggplot(ylg_timing,
                         aes(y=time,
                             x=run_index))+
  ...
  geom_line(data=comm.data.pred,
              aes(x=run_index, y=time), size=1, colour=colour_labels[1])+
  geom_ribbon(data=comm.data.pred,
              aes(ymin=y.lower, ymax=y.upper), alpha=0.4, fill=colour_labels[1])+
  geom_line(data=foster.data.pred,
              aes(x=run_index, y=time), size=1, colour=colour_labels[2])+
  geom_ribbon(data=foster.data.pred,
              aes(ymin=y.lower, ymax=y.upper), alpha=0.4, fill=colour_labels[2])+
   labs(x='Run number', y='Run completion time\n(in minutes)', color='Sample')+
...
conditions_timing<-ylg_timing %>% filter(!condition=="Mock")
condition_model=lme(time ~ condition, method="ML", random = ~1|sub, data=conditions_timing)
run_index_model=lme(time ~ run_index, method="ML", random = ~1|sub, data=conditions_timing)

anova(condition_model,run_index_model)
condition_group_model=lme(time ~ condition+group, method="ML", random = ~1|sub, data=conditions_timing)
condition_intgroup_model=lme(time ~ condition*group, method="ML", random = ~1|sub, data=conditions_timing)
anova(condition_model,condition_group_model,condition_intgroup_model)
# 
summary(condition_model)

nullcondmodel=lme(time ~ 1, method="ML", random = ~1|sub, data=conditions_timing)
condition_runindex_model=lme(time ~ condition+run_index, method="ML", random = ~1|sub, data=conditions_timing)
condition_intrunindex_model=lme(time ~ condition*run_index, method="ML", random = ~1|sub, data=conditions_timing)
condition_intrunindex_group_model=lme(time ~ condition*run_index+group, method="ML", random = ~1|sub, data=conditions_timing)
condition_intrunindex_intgroup_model=lme(time ~ condition*run_index*group, method="ML", random = ~1|sub, data=conditions_timing)
anova(nullcondmodel,condition_model,condition_runindex_model,
...
# best model takes into account the main effects of condition
summary(condition_model)

# Time plotted across all runs by condition and sample
if(resave_plots){condition.plot <- ylg_timing %>%
  group_by(group, condition, run_index) %>% 
...
if(resave_plots){condition_plot<-ggplot(condition.plot,
                  aes(x=run_index, y=mean, color=group)) +
  geom_point(position=position_dodge(.1), size=3) +
  geom_line(aes(group=group), size=1) +
  geom_errorbar(aes(ymax=mean + ci, ymin=mean - ci),
  ...
  theme(axis.line = element_line(colour = "black"),
        ...
        legend.position="bottom")
condition_plot
ggsave(filename=paste0(workdir,"../bids_data/derivatives/strategic_connectivity/graphs/time_by_condition_runindex_group.png"),
       plot=condition_plot, width=7, height=5, units='in', dpi=300)}

# Predicted model
# generate community sample models
commnullmodel=lme(time ~ 1, method="ML", random = ~1|sub, data=(conditions_timing %>%
                                                                    filter(group=="community")))
commmodel=lme(time ~ condition, method="ML", random = ~1|sub, data=(conditions_timing %>%
                                                                    filter(group=="community")))
anova(commnullmodel,commmodel)
summary(commmodel)

# generate foster sample models
fosternullmodel=lme(time ~ 1, method="ML", random = ~1|sub, data=(conditions_timing %>%
                                                                    filter(group=="foster")))
fostermodel=lme(time ~ condition, method="ML", random = ~1|sub, data=(conditions_timing %>%
                                                                    filter(group=="foster")))
anova(fosternullmodel,fostermodel)
summary(fostermodel)
condition<-c("Alone","Peer","Exclusion")
comm.data.pred = cbind.data.frame(condition)
y.pred = predict(commmodel,
                   comm.data.pred,
                   level=0)
comm.data.pred = cbind.data.frame(comm.data.pred,
...
foster.data.pred = cbind.data.frame(condition)
y.pred = predict(fostermodel,
                   foster.data.pred,
                   level=0)
foster.data.pred = cbind.data.frame(foster.data.pred,
...
if(resave_plots){model_time_by_condition<-ggplot(conditions_timing,
                                aes(y=time,
                                    x=condition,
                                    group=1))+
  geom_line(data=conditions_timing,
            aes(colour=group, group=sub),size=.2,alpha=0.2)+
  geom_point(data=conditions_timing,
             aes(colour=group, group=sub),size=1,alpha=0.2)+
  geom_line(data=foster.data.pred,
            aes(x=condition, y=time,group=1), size=1, colour=colour_labels[2])+
  geom_ribbon(data=foster.data.pred,
              aes(ymin=y.lower, ymax=y.upper,group=1), alpha=0.4, fill=colour_labels[2])+
  geom_line(data=comm.data.pred,
            aes(x=condition, y=time,group=1), size=1, colour=colour_labels[1])+
  geom_ribbon(data=comm.data.pred,
              aes(ymin=y.lower, ymax=y.upper,group=1), alpha=0.4, fill=colour_labels[1])+
  scale_fill_manual(values = group_colors)+
  scale_colour_manual(values = group_colors)+
  labs(x='Condition', y='Run completion time\n(in minutes)', color='Sample')+
  theme_bw() +
  theme_minimal(base_size = 20, base_family = "Arial") +
  theme(axis.line = element_line(colour = "black"),
        panel.grid.major = element_blank(),
        ...
        panel.background = element_blank(),
        legend.position="bottom")
model_time_by_condition
ggsave(filename=paste0(workdir,"../bids_data/derivatives/strategic_connectivity/graphs/model_time_by_condition.png"),
       plot=model_time_by_condition, width=7, height=5, units='in', dpi=300)}
...
if(run_group_differences){
    ...
    social_groupdiff_models<-mclapply(X=as.character(as.list(social_cois$coi)), 
                                      ...
                                      FUN=function(coi_name, demodata, coidat){
                                        ...
                                        null_mod=(lme(cor ~ 1,
                                                       ...
                                                       random = ~1|sub,
                                                       data=adf))
                                        group_coi_mod=(lme(cor ~ group,
                                                           method="ML",
                                                           random = ~1|sub,
                                                           data=adf))
                                        mod_comp<-anova(null_mod,group_coi_mod)
                                        if (mod_comp$'p-value'[2]<.05 &
                                            mod_comp$AIC[2]<mod_comp$AIC[1] &
                                           anova(group_coi_mod)$`p-value`[2]<.05){
                                          ...
                                          fostereffect<-round(summary(group_coi_mod)$coefficients$fixed[2],3)
                                          mod_type <- "group difference"
                                          retDF <- cbind(coiname,fostereffect,chisq,pval,AIC,nullAIC,mod_type)
  ...
  save(social_groupdiff_modelsDT, file= paste0("/Volumes/TDS/bids_data/derivatives/strategic_connectivity/connectivity_models/",
                                               "social_groupdiff_modelsDT.RDS"))
} else {
  social_groupdiff_modelsDT<-load(file= paste0("/Volumes/TDS/bids_data/derivatives/strategic_connectivity/connectivity_models/",
                                               "social_groupdiff_modelsDT.RDS"))
}
...
if(run_condition){
    ...
    condition_models<-mclapply(X=as.character(as.list(social_cois$coi)), 
                              ...
                              FUN=function(coi_name, demodata, coidat){
                                adf<-merge(as.data.table(filter(coidat,coi==coi_name)),
                                           ...
                                           by='sub',
                                           allow.cartesian=T)
                                beh_mod=lme(time ~ condition,
                                            method="ML",
                                            random = ~1|sub,
                                            data=adf)
                                coi_mod=lme(time ~ condition+cor,
                                            method="ML",
                                            random = ~1|sub,
                                            data=adf)
                                coi_int_mod=lme(time ~ condition*cor,
                                                method="ML",
                                                random = ~1|sub,
                                                data=adf)
                                coi_int_group_mod=lme(time ~ condition*cor*group,
                                                method="ML",
                                                random = ~1|sub,
                                                data=adf)
                                mod_comp<-anova(beh_mod,coi_mod,coi_int_mod,coi_int_group_mod)
  ...
  save(condition_modelsDT, file= paste0("/Volumes/TDS/bids_data/derivatives/strategic_connectivity/",
                                     "connectivity_models/condition_modelsDT.RDS"))
} else {
  load(file= paste0("/Volumes/TDS/bids_data/derivatives/strategic_connectivity/",
                    "connectivity_models/condition_modelsDT.RDS"))
}
...
prepheat<-read.csv(file=paste0(workdir,"../bids_data/derivatives/strategic_connectivity/tables/",
                                          "strategy_heatmap_int_percent.csv"),header=TRUE,stringsAsFactors = FALSE)

strategy_heatmap<-ggplot(data = prepheat,
       aes(x=seed_community, y=target_community, fill=percent_interaction)) + 
 ...
 coord_fixed()
ggsave(filename=paste0(workdir,"../bids_data/derivatives/strategic_connectivity/graphs/strategy_corrmatrix_int_percent.png"),
...
assign(paste0("foster_coi_graph"),
           ggplot(data=NULL,
             ...
             geom_line(data=(adf %>% filter(group=="foster")),
                       aes(group=sub),size=.2,alpha=0.1, colour=group_colors[2])+
             geom_point(data=(adf %>% filter(group=="foster")),
                        aes(group=sub),size=.7,alpha=0.1, colour=group_colors[2])+
...
Unpack condition models
```{r}
redoprepheat=FALSE

load(file = "/Volumes/TDS/bids_data/derivatives/strategic_connectivity/connectivity_data/Social_noWBVR_FishersZ_coisDT.RDS")

load(file= paste0("/Volumes/TDS/bids_data/derivatives/strategic_connectivity/connectivity_models/",
                                               "social_groupdiff_modelsDT.RDS"))

load(file= paste0("/Volumes/TDS/bids_data/derivatives/strategic_connectivity/connectivity_models/",
                    "condition_modelsDT.RDS"))

# How many coi models that differ between groups also explain variance in condition effects on time?
overlapping_cois<-intersect(x=(social_groupdiff_modelsDT %>% filter(!is.na(mod_type)))$coiname,
                            y=(condition_modelsDT %>% filter(!is.na(mod_type)))$coiname)
condition_group_coi_models<-condition_modelsDT %>% filter(coiname %in% overlapping_cois)

# Check out sig. condition coi models 
# MAIN
mclapply(X=as.character(as.list(condition_modelsDT%>%filter(mod_type=="coi main model"))$coiname), 
         demodata=conditions_timing,
         coidat=social_coisDT,
         mc.cores=numcores,
         FUN=function(coi_name, demodata, coidat){
           adf<-merge(as.data.table(filter(coidat,coi==coi_name)),
                      ...
                      allow.cartesian=T)
           cat(coi_name)
           summary(lme(time ~ condition+cor, method="ML",random = ~1|sub,data=adf))})

# INTERACTION
mclapply(X=as.character(as.list(condition_modelsDT%>%filter(mod_type=="coi int model"))$coiname), 
         demodata=conditions_timing,
         coidat=social_coisDT,
         mc.cores=numcores,
         FUN=function(coi_name, demodata, coidat){
...
mclapply(X=as.character(as.list(condition_modelsDT%>%filter(mod_type=="coi main model"))$coiname), 
         ...
         FUN=function(coi_name, demodata, coidat){
           adf<-merge(as.data.table(filter(coidat,coi==coi_name)),
                      ...
                      by='sub',
                      allow.cartesian=T)
           groupmodel=lme(time ~ condition+cor, method="ML", random = ~1|sub, data=adf)
           #summary(groupmodel)
           ...
           assign(paste0("coi_condition_predictions"),
                  lapply(X=cor,
                         FUN=function(COR){
                           assign(paste0("frame",COR),
                                  data.frame(condition=condition,
                                             cor=COR))
                           assign(paste0("frame",COR,"predvalue"),
                                  predict(groupmodel, get(paste0("frame",COR)), level=0))
                           assign(paste0("designmat",COR),
                                  model.matrix(eval(eval(groupmodel$call$fixed)[-2]),
                                               get(paste0("frame",COR))[-3]))
                           assign(paste0("frame",COR,"SDvalue"),
                                  sqrt(diag(get(paste0("designmat",COR)) 
                                            %*% groupmodel$varFix 
           ...
           assign(paste0("coi_condition_graph"),
                  ggplot(data=NULL,
                         aes(x=condition,
                             y=time,
                             group=1)) +
                    ...
                    geom_point(data=(adf %>% filter(group=="foster")),
                               aes(group=sub),size=.7,alpha=0.1, colour=group_colors[2])+
                    geom_line(data=as.data.frame(coi_condition_predictions[[1]]),
                              aes(x=condition,
                                  y=time,
                                  group=1),
                              size=1,
                              colour=purple_colours[1]) +
                    geom_ribbon(data=as.data.frame(coi_condition_predictions[[1]]),
                                aes(ymin=lowerCIvalue,
                                    ymax=upperCIvalue,
                                    group=1),
                                alpha=0.2, fill=purple_colours[1]) +
                    geom_line(data=as.data.frame(coi_condition_predictions[[2]]),
                              aes(x=condition,
                                  y=time,
                                  group=1),
                              size=1,
                              colour=purple_colours[2]) +
                    geom_ribbon(data=as.data.frame(coi_condition_predictions[[2]]),
                                aes(ymin=lowerCIvalue,
                                    ymax=upperCIvalue,
                                    group=1),
                                alpha=0.2, fill=purple_colours[2]) +
                    geom_line(data=as.data.frame(coi_condition_predictions[[3]]),
                              aes(x=condition,
                                  y=time,
                                  group=1),
                              size=1,
                              colour=purple_colours[3]) +
                    geom_ribbon(data=as.data.frame(coi_condition_predictions[[3]]),
                                aes(ymin=lowerCIvalue,
                                    ymax=upperCIvalue,
                                    group=1),
                                alpha=0.2, fill=purple_colours[3]) +
                    ...
                    theme(axis.line.x = element_line(colour = "black", size=.5),
                          ...
                          panel.background = element_blank(),
                          legend.position="none"))
           get(paste0("coi_condition_graph"))
           ggsave(filename=paste0(workdir,"../bids_data/derivatives/strategic_connectivity/",
                                  "graphs/condition_coi_predicted_graphs/",
                                  "coi_condition_graph_",coi_name,".png"),
                  plot=coi_condition_graph, width=6, height=5, units='in', dpi=300)
         })
...
condition_model=lme(time ~ condition, method="ML", random = ~1|sub, data=conditions_timing)
condition<-c("Alone","Peer","Exclusion")
data.pred = cbind.data.frame(condition)
y.pred = predict(condition_model,
                   data.pred,
                   level=0)
data.pred = cbind.data.frame(data.pred,
                               y.pred)
scale = 1.96
designmat<-model.matrix(eval(eval(condition_model$call$fixed)[-2]),
                          data.pred[-3]) #make design matrix
SDvalue<-sqrt(diag(designmat %*% condition_model$varFix %*% t(designmat))) #calculate standard deviation for each point for each model
y.lower<-y.pred-(scale*SDvalue) #calculate confidence intervals - lower
y.upper<-y.pred+(scale*SDvalue) #calculate confidence intervals - upper
data.pred = cbind.data.frame(data.pred,
                               y.lower,
                               y.upper)
data.pred$time<-data.pred$y.pred

# Time plotted across all runs by condition and sample
condition.plot <- ylg_timing %>%
  group_by(condition) %>% 
  filter(!condition=="Mock") %>%
  summarize(mean = mean(time),
            sd = sd(time),
...
condition_plot<-ggplot(condition.plot,
                  aes(x=condition, y=time)) +
  geom_point(data=condition.plot,color="dodgerblue", size=0) +
  geom_errorbar(data=condition.plot,aes(ymax=time + ci, ymin=time - ci),
                width=0.5,color="dodgerblue") +
  geom_line(data=data.pred,
            aes(x=condition, y=time,group=1), size=1, colour="dodgerblue")+
  geom_ribbon(data=data.pred,
              aes(ymin=y.lower, ymax=y.upper,group=1), alpha=0.4, fill="dodgerblue")+
   geom_line(data=conditions_timing,
             aes(group=sub,colour=sub),size=.2,alpha=0.5)+
   geom_point(data=conditions_timing,
              aes(group=sub,colour=sub),size=.5,alpha=0.5)+
  labs(x='Condition', y='Run completion time\n(in minutes)') +
  ylim(2,2.75)+
  theme_kate()
condition_plot
ggsave(filename=paste0(workdir,"../bids_data/derivatives/strategic_connectivity/graphs/ORI/condition_behavior_model_withsubs.png"),
       plot=condition_plot, width=5, height=4, units='in', dpi=300)


coi_name<-as.character(as.list(condition_modelsDT%>%filter(mod_type=="coi int model"))$coiname)[4]
demodata<-conditions_timing
coidat<-social_coisDT
...
cor=seq((mean(adf$cor)-sd(adf$cor)),
 ...
 condition<-c("Alone","Peer","Exclusion")
          ...
          condition_conn_plot<-ggplot(data=NULL,
                         aes(x=condition,
                             y=time,
                             group=1)) +
                    geom_line(data=adf,
                              aes(group=sub,colour=cor),size=.2,alpha=0.5)+
                    geom_point(data=adf,
                               aes(group=sub,colour=cor),size=.5,alpha=0.5)+
              scale_color_gradient(low = "dodgerblue", high = purple_colours[3])+
                    labs(x='Condition', y='Run completion time\n(in minutes)') +
                    ylim(2,2.75)+
                    geom_line(data=as.data.frame(coi_condition_predictions[[1]]),
                              aes(x=condition,
                    ...
                    geom_ribbon(data=as.data.frame(coi_condition_predictions[[3]]),
                                aes(ymin=lowerCIvalue,
                                    ymax=upperCIvalue,
                                    group=1),
                                alpha=0.3, fill=purple_colours[3]) +
                    theme_kate()
condition_conn_plot
          ggsave(filename=paste0(workdir,"../bids_data/derivatives/strategic_connectivity/graphs/ORI/condition_connectivity_model_withsubs_bothconn.png"),plot=condition_conn_plot, width=5, height=4, units='in', dpi=300)
