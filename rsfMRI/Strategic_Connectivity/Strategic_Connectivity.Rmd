...
time.plot <-
  ...
  summarize(mean = mean(time),
            ...
            se = sd/sqrt(n),
            ci=qt(0.975, df=n-1)*se)
time.plot$condition = factor(time.plot$condition,levels(time.plot$condition)[c(3,1,4,2)])
...
ylg_time_by_learning_graph<-ggplot(ylg_timing,
                         aes(y=time,
                             x=run_index))+
...
condition_runindex_group_model=lme(time ~ condition+run_index+group, method="ML", random = ~1|sub, data=ylg_timing)
...
anova(nullmodel,condition_model,condition_runindex_model,
      condition_intrunindex_model,condition_runindex_group_model,
...
if(!file.exists("/Volumes/TDS/bids_data/derivatives/strategic_connectivity/gordon_coisDT.RDS")){
  ...
  system.time(gordon_cois<- mclapply(as.list(as.character(sublist$sub))[1:10],
    ...
    social_groupdiff_models<-mclapply(X=as.character(as.list(social_cois$coi)), 
      ...
      FUN=function(coi_name, demodata, coidat){
        ...
        group_mod=(lme(cor ~ group,
                      ...
                      random = ~1|sub,
                      data=adf))
        group_coi_mod=(lme(cor ~ group*time,
                         method="ML",
                         random = ~1|sub,
                         data=adf))
        mod_comp<-anova(null_mod,group_mod,group_coi_mod)
...
if(rerun_models){
  # Identify parcels where connectivity differs by group membership
  system.time(
    social_strategy_models<-mclapply(X=as.character(as.list(social_cois$coi)), 
      demodata=ylg_timing,
      coidat=social_coisDT,
      mc.cores=numcores,
      FUN=function(coi_name, demodata, coidat){
        adf<-merge(as.data.table(filter(coidat,coi==coi_name)),
                   ...
                   allow.cartesian=T) %>%
        filter(!group=="juvjus")
        beh_mod=lme(time ~ condition+run_index,
                      method="ML",
                      random = ~1|sub,
                      data=adf)
        group_mod=lme(time ~ condition+run_index+group,
                      method="ML",
                      random = ~1|sub,
                      data=adf)
        coi_mod=lme(time ~ condition+run_index+group+cor,
                         method="ML",
                         random = ~1|sub,
                         data=adf)
        coi_int_mod=lme(time ~ condition+run_index+group*cor,
                         method="ML",
                         random = ~1|sub,
                         data=adf)
        mod_comp<-anova(group_mod,coi_mod,coi_int_mod,beh_mod)
