...
povline_impute=1 # if this is set to 1 (true), household size will be imputed with the mean for each group, rounded down (4 for the community sample and 5 for the dhs and jj samples), affecting the income:needs ratio calculation; otherwise set to 0
...
df_aces$ACE_neglect_num= ifelse(df_aces$ACE_ace_neglect_n_missing<=1, mapvalues(df_aces$ACE_ace_neglect_score, from=c("0","0.5","1.0"), to=c("0", "1","2")), "NA")
...
## ACES
df_aces_mi = df_aces %>% group_by(group) %>% summarise(abuse_mi = sum(ACE_ace_abuse_n_missing, na.rm=TRUE), 
...
df = df %>% rename("enough"=PAL2_5, "hhsize"=PAL2_7)
...
df_pairs=cbind.data.frame(df$ACE_ace_abuse_score, df$ACE_ace_neglect_score, df$ACE_ace_hh_dysf_score, df$income_to_needs, df$group)
colnames(df_pairs)=c("A","N","H","ItN", "group")
...
df_binary$group=factor(df_binary$group, levels=c("1","2","3"), labels=c("comm","dhs","jj"))
df_binary$A=factor(df_binary$A, levels=c("0","1"), labels=c("","A"))
df_binary$N=factor(df_binary$N, levels=c("0","1"), labels=c("","N"))
df_binary$H=factor(df_binary$H, levels=c("0","1"), labels=c("","H"))
df_binary$P=factor(df_binary$P, levels=c("0","1"), labels=c("","P"))
