...
null_models<-mclapply(X=as.list(gordon_cois$coi[1:5]), 
...
abuse_models<-mclapply(X=as.list(gordon_cois$coi[1:5]), 
      demodata=demographicsDF,
      coidat=gordon_coisDT,
      mc.cores=1,
      FUN=function(coi_name, demodata, coidat){
...
neglect_abuse_models<-mclapply(X=as.list(gordon_cois$coi[1:5]), 
      demodata=demographicsDF,
      coidat=gordon_coisDT,
      mc.cores=1,
      FUN=function(coi_name, demodata, coidat){
...
neglect_abuse_int_models<-mclapply(X=as.list(gordon_cois$coi[1:5]), 
      demodata=demographicsDF,
      coidat=gordon_coisDT,
      mc.cores=1,
      FUN=function(coi_name, demodata, coidat){
...
adf<-merge(as.data.table(filter(gordon_coisDT,coi=="aseg_17_aseg_18")),
       ...
       neglect_abuse_int_mod=(lme(cor ~ neglect*abuse,
        ...
        for (i in 1:length(model_fit_stats)){
         if (mod_comp$'p-value'[4]<.05 &
          ...
          retDF<-cbind(coiname, chisq, pval, AIC, nullAIC, nullBIC, logLik, nulllogLik, mod_type, mod)
