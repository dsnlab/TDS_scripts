...
if(rerun_modelCompare){
  system.time(
    gordon_models<-mclapply(X=as.list(gordon_cois$coi[1:20]), 
      demodata=demographicsDF,
      coidat=gordon_coisDT,
      mc.cores=1,
      FUN=function(coi_name, demodata, coidat){
       ...
       if (mod_comp$'p-value'[4]<.05 &
            mod_comp[[model_fit_stats[i]]][4]<mod_comp[[model_fit_stats[i]]][3] &
            mod_comp[[model_fit_stats[i]]][4]<mod_comp[[model_fit_stats[i]]][2] &
             mod_comp[[model_fit_stats[i]]][4]< mod_comp[[model_fit_stats[i]]][1]){
          coiname<- as.character(coi_name); criteria <- model_fit_stats[i]
          chisq <- mod_comp[4,8]; pval <- round(mod_comp[4,9],4)
          AIC <- mod_comp[4,4]; nullAIC <- mod_comp[1,4]
          BIC <- mod_comp[4,5]; nullBIC <- mod_comp[1,5]
          logLik <- mod_comp[4,6]; nulllogLik <- mod_comp[1,6]
          mod <- list(neglect_abuse_main_mod); mod_type <- "neglect and abuse interaction model"
          retDF<-cbind(coiname, criteria, chisq, pval, AIC, nullAIC, nullBIC, logLik, nulllogLik, mod_type, mod)
          as.data.table(retDF)
          #assign(paste0("retDF_",model_fit_stats[i]), as.data.table(retDF))
        } else if (mod_comp$'p-value'[3]<.05 &
                   mod_comp[[model_fit_stats[i]]][3]<mod_comp[[model_fit_stats[i]]][2] &
                   mod_comp[[model_fit_stats[i]]][3]<mod_comp[[model_fit_stats[i]]][1]) {
          coiname<- as.character(coi_name); criteria <- model_fit_stats[i]
          chisq <- mod_comp[3,8]; pval <- round(mod_comp[3,9],4)
          ...
          retDF<-cbind(coiname, criteria, chisq, pval, AIC, nullAIC, nullBIC, logLik, nulllogLik, mod_type, mod)
          as.data.table(retDF)
          #assign(paste0("retDF_",i), as.data.table(retDF))
        } else if (mod_comp$'p-value'[2]<.05 &
                   mod_comp[[model_fit_stats[i]]][2]<mod_comp[[model_fit_stats[i]]][1]){
          coiname<- as.character(coi_name); criteria <- model_fit_stats[i]
          chisq <- mod_comp[2,8]; pval <- round(mod_comp[2,9],4)
  ...
  rm(gordon_models);gc()
          # saveRDS(paste0("retDF",i), paste0("/Volumes/TDS/bids_data/derivatives/Cheng_SAP/retDF", i, ".RDS"))
    }

```


```{r Kate's analysis code}
rerun_models=TRUE

if(rerun_models){
  system.time(
    gordon_models<-mclapply(X=as.list(gordon_cois$coi[1]), 
      demodata=demographicsDF[1:10,],
      coidat=gordon_coisDT,
      mc.cores=1,
      FUN=function(coi_name, demodata, coidat){
        adf<-merge(as.data.table(filter(coidat,coi==coi_name)),
