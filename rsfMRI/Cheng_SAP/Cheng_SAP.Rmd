...
collectAndCorTimecourses <- function(sub, parcels, scrubbingThreshold, sub_base_dir) {
  ...
  subjDF <- subjDF %>%
    filter(grepl("aseg", coi)) #this filter step selects connections to amygdala and hippocampus only
}
...
##Identify COI seed
gordon_modelsDT$seed=gsub('(^\\w{4,9}_\\d{2,3})_.*','\\1',gordon_modelsDT$coiname[1:100])
...
df_gordon_models= data.frame(cbind("ParcelID"=unlist(gordon_modelsDT$ParcelID), "seed"=unlist(gordon _modelsDT$seed), "mod_type"=unlist(gordon_modelsDT$mod_type), "Community"=unlist(gordon_modelsDT$Community))) 
...
# Table 
# turn into a table and calculate percentages

# for each seed, find the number of connections that were explained by non-null models. then find the percentage of connections for each model explained by each network type. 
