...
```{r}
marijuana_everused<-long_survey_data %>%
  filter(grepl("YRBS_20",item)) # 0 = No; 1 = Yes

marijuana_past30days<-long_survey_data %>%
  filter(grepl("YRBS_22",item)) # 0 to 6 scale with 6 = 30 days

marijuana_likelyusenext6months <-long_survey_data %>%
  filter(grepl("CARE_EI_10A",item)) # 1 to 7 scale with 7 = extremely likely

marijuana_willingness <-long_survey_data %>%
