...
TDS1_SESS1_POST_Marijuana<-c("CARE_WE_1A","CARE_EI_1A")
...
distinct(SID,item,value,survey_name,.keep_all = FALSE) %>% 
  ...
  mutate(mj_everused=YRBS_20,
         ...
         mj_willing=ifelse(!is.na(CARE_WE_10A),CARE_WE_10A,
                           ifelse(!is.na(CARE_WE_1A),CARE_WE_1A,
                                  NA)),
         mj_expected=ifelse(!is.na(CARE_EI_10A),CARE_EI_10A,
                           ifelse(!is.na(CARE_EI_1A),CARE_EI_1A,
                                  NA)),
...
###### Alcohol ########
YRBS_Alcohol<-c("A1", "A9", "A6")

alcohol_everused<-long_survey_data %>%
  filter(grepl("YRBS_20",item)) # 0 = No; 1 = Yes

alcohol_past30days<-long_survey_data %>%
  filter(grepl("YRBS_22",item)) # 0 to 6 scale with 6 = 30 days

alcohol_likelyusenext6months <-long_survey_data %>%
  filter(grepl("CARE_EI_10A",item)) # 1 to 7 scale with 7 = extremely likely

alcohol_willingness <-long_survey_data %>%
  filter(grepl("CARE_WE_10A",item)) # 1 to 7 scale with 7 = extremely willing
