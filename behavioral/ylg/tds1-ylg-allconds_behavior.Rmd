...
null.model <- glmer(go ~ (1|scores.SID), data=df_fMRI_all_stops, family='binomial')
run.model <- glmer(go ~ run_index + (1|scores.SID), data=df_fMRI_all_stops, family='binomial')
anova(null.model, run.model)     
#            Df   AIC   BIC  logLik deviance  Chisq Chi Df Pr(>Chisq)    
# null.model  2 9765.2 9779.1 -4880.6   9761.2                         
# run.model   7 9766.9 9815.6 -4876.4   9752.9 8.2861      5     0.1412
summary(run.model) 
# Fixed effects:
#              Estimate Std. Error z value Pr(>|z|)    
# (Intercept) -0.57696    0.11899  -4.849 1.24e-06 ***
# run_index4  -0.10507    0.08505  -1.235    0.217    
# run_index5   0.01076    0.08457   0.127    0.899    
# run_index6   0.12079    0.08423   1.434    0.152    
# run_index7  -0.06497    0.08488  -0.765    0.444    
# run_index8  -0.01078    0.08465  -0.127    0.899    
...
null.model <- glmer(go_cr ~ (1|scores.SID), data=df_fMRI_all_stops, family='binomial')
run.model <- glmer(go_cr ~ run_index + (1|scores.SID), data=df_fMRI_all_stops, family='binomial', control=glmerControl(optimizer="bobyqa"))
anova(null.model, run.model) 
#            Df    AIC    BIC  logLik deviance  Chisq Chi Df Pr(>Chisq)  
# null.model  2 7525.5 7539.5 -3760.8   7521.5                         
# run.model   7 7531.8 7580.5 -3758.9   7517.8 3.7795      5     0.5816
summary(run.model) 
# Fixed effects:
#             Estimate Std. Error z value Pr(>|z|)    
...
null.model <- glmer(stop ~ (1|scores.SID), data=df_fMRI_all_stops, family='binomial')
run.model <- glmer(stop ~ run_index + (1|scores.SID), data=df_fMRI_all_stops, family='binomial')
anova(null.model, run.model)
#            Df   AIC   BIC  logLik deviance  Chisq Chi Df Pr(>Chisq)    
# null.model  2 10102 10116 -5049.0    10098                           
# run.model   7 10100 10149 -5043.2    10086 11.595      5    0.04077 *
summary(run.model)
# Fixed effects:
#              Estimate Std. Error z value Pr(>|z|)    
# (Intercept) 0.242123   0.106960   2.264   0.0236 *
# run_index4  0.178424   0.082830   2.154   0.0312 *
# run_index5  0.140405   0.082730   1.697   0.0897 .
# run_index6  0.003405   0.082471   0.041   0.9671  
# run_index7  0.185360   0.082848   2.237   0.0253 *
# run_index8  0.181893   0.082841   2.196   0.0281 *
...
null.model <- glmer(stop_nocar ~ (1|scores.SID), data=df_fMRI_all_stops, family='binomial')
run.model <- glmer(stop_nocar ~ run_index + (1|scores.SID), data=df_fMRI_all_stops, family='binomial')
anova(null.model, run.model)
#            Df    AIC    BIC  logLik deviance  Chisq Chi Df Pr(>Chisq)  
# null.model  2 9209.0 9223.0 -4602.5   9205.0                         
# run.model   7 9217.4 9266.1 -4601.7   9203.4 1.6825      5     0.8911
summary(run.model)
# Fixed effects:
#             Estimate Std. Error z value Pr(>|z|)    
# (Intercept) -0.97973    0.07491 -13.078   <2e-16 ***
# run_index4   0.06572    0.08785   0.748    0.454    
# run_index5   0.04273    0.08805   0.485    0.627    
# run_index6  -0.02760    0.08871  -0.311    0.756    
# run_index7   0.05809    0.08792   0.661    0.509    
# run_index8   0.01563    0.08830   0.177    0.860      
...
go_by_sid = df_fMRI_all_stops %>% group_by(SID, run_index, condition) %>% summarize(mean_go = mean(go))
...
go_cr_by_sid = df_fMRI_all_stops %>% group_by(SID, run_index, condition) %>% summarize(mean_go_cr = mean(go_cr))
plot_group_go_cr = go_cr_by_sid %>% group_by(run_index, condition) %>% summarize(mean = mean(mean_go_cr), 
                                                                    sd = sd(mean_go_cr), 
                                                                    n=66, # entered manually (could not be calculated because 127 has 140 trials, whereas all other subjects have 160 trials)
                                                                    se=sd/sqrt(n), 
                                                                    ci=qt(0.975, df=n-1)*se) 
...
stop_by_sid = df_fMRI_all_stops %>% group_by(SID, run_index, condition) %>% summarize(mean_stop = mean(stop))
...
stop_nocar_by_sid = df_fMRI_all_stops %>% group_by(SID, run_index, condition) %>% summarize(mean_stop_nocar = mean(stop_nocar))
...
# go             1    0.2  0.2106    1.12   0.29
# Residuals   7938 1492.4  0.1880               
