...
##Collect data
ylg_data<-read.csv(paste(ylg_folder,'ylg-behavior-N174-tds123.csv',sep=''))
...
write.csv(events_by_sid, paste(ylg_folder,'tds_events_by_sid_121718.csv',sep=''))
...
df <- merge(demo_filtered_context, ylg_filtered, by = 'SID')
...
run.model <- glmer(go ~ run_index + (1 + condition|SID), data=df_fMRI, family='binomial')
...
null.model <- glmer(go ~ (1|SID), data=df_fMRI, family='binomial')
condition.model <- glmer(go ~ condition + (1 + condition|SID), data=df_fMRI, family='binomial')
...
condition.model <- glmer(go ~ condition + (1+ condition|SID), data=social_df, family='binomial')
anova(null.model, condition.model)  
#                Df    AIC    BIC  logLik deviance Chisq Chi Df Pr(>Chisq)    
...
run.model <- glmer(go_cr ~ run_index + (1+ condition|SID), data=df_fMRI, family='binomial', control=glmerControl(optimizer="bobyqa"))
anova(null.model, run.model) 
#            Df    AIC    BIC  logLik deviance  Chisq Chi Df Pr(>Chisq)  
...
interaction.model <- glm(BIS.15_bis_total_score ~ go*condition + (1|SID), data=df_RWR_fMRI)
