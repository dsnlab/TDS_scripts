...
##Collect data
ylg_data<-read.csv(paste(ylg_folder,'tds_events_by_sid_11.19.18.csv',sep=''))
...
demo_filtered_context <- demo %>% filter(SID < 300) %>% 
  filter(!SID %in% no_session2) %>% 
  filter(!SID %in% behaviorals)%>%
  filter(!SID %in% pilot_cyberball) %>%
  filter(!SID %in% motion_dropout)
...
PDS_mean_scores <- demo_filtered_context %>% distinct(SID, gender, PDS_pds_gender_score, PDS_mean_score) %>% filter(!grepl("189|158|160|174", SID)) 
write.csv(PDS_mean_scores, file = 'tds2_PDS_mean_score_n68.csv')
```
#############
## TABLE 1 ##
#############
...
age_by_sex<-demo_filtered_context %>% filter(!is.na(age)) %>% group_by(gender) %>% 
  summarize(mean = mean(age, na.rm=T), sd = sd(age, na.rm=T), 
            ...
            n = n(), se = sd/sqrt(n), ci=qt(0.975, df=n-1)*se) # n=37 females

age_combined<-demo_filtered_context %>% filter(!is.na(age)) %>% 
  summarize(mean = mean(age, na.rm=T), sd = sd(age, na.rm=T), 
            min = min(age, na.rm=T), max = max(age, na.rm=T),
...
#Collapsed across runs within social context, however, teens stop less in both social context, than they do when they play alone.
...
###########################################################################################################
ROI with task behavior and RWR behavior
###########################################################################################################

```{r}

#import ROI data frame
#merge with other behavior and rwr data frame
...
null.model <- glmer(BIS.15_bis_total_score ~ Nacc + (1|SID), data=df_RWR_fMRI)
main.model <- glmer(BIS.15_bis_total_score ~ Nacc + condition + (1|SID), data=df_RWR_fMRI)
interaction.model <- glmer(BIS.15_bis_total_score ~ Nacc*condition + (1|SID), data=df_RWR_fMRI)
anova(null.model, main.model)
anova(null.model, interaction.model)
summary(null.model)


#see link Nacc with behavior
null.model <- glmer(go ~ Nacc + (1|SID), data=df_RWR_fMRI)
main.model <- glmer(go ~ Nacc + condition + (1|SID), data=df_RWR_fMRI)
interaction.model <- glmer(go ~ Nacc*condition + (1|SID), data=df_RWR_fMRI)
anova(null.model, main.model)
anova(null.model, interaction.model)
summary(null.model)
...
```{r}
# Run effects on the proportion of Go decisions
null.model <- lm(time ~ (1|SID), data=df_fMRI)
run.model <- lm(time ~ run_index + (1|SID), data=df_fMRI)
anova(null.model, run.model)     
#   Res.Df    RSS Df Sum of Sq      F    Pr(>F)    
# 1   8399 151.42                                  
# 2   8394 147.98  5     3.439 39.015 < 2.2e-16 ***
summary(run.model) 
# Fixed effects:
#              Estimate Std. Error t value Pr(>|t|)    
# (Intercept)  2.314269   0.003549 652.169  < 2e-16 ***
# run_index4   0.030740   0.005018   6.125 9.45e-10 ***
# run_index5  -0.018605   0.005018  -3.707 0.000211 ***
# run_index6  -0.015427   0.005018  -3.074 0.002118 ** 
# run_index7  -0.011105   0.005018  -2.213 0.026942 *  
# run_index8  -0.034732   0.005018  -6.921 4.82e-12 ***

#Game time decreased across runs-- getting better.

# Condition effects on the proportion of Go decisions
null.model <- lm(time ~ (1|SID), data=df_fMRI)
condition.model <- lm(time ~ condition + (1|SID), data=df_fMRI)
anova(null.model, condition.model)  
#   Res.Df    RSS Df Sum of Sq      F    Pr(>F)    
# 1   8399 151.42                                  
# 2   8397 149.04  2    2.3797 67.037 < 2.2e-16 ***
summary(condition.model)  
#  Fixed effects:
#              Estimate Std. Error t value Pr(>|t|)    
# (Intercept)  2.329639   0.002518 925.292   <2e-16 ***
# condition3  -0.032386   0.003561  -9.096   <2e-16 ***
# condition4  -0.038288   0.003561 -10.753   <2e-16 ***

#consistent collapsed across runs

social_df <- df_fMRI %>% filter(condition != "2")
null.model <- lm(time ~ (1|SID), data=social_df)
condition.model <- lm(time ~ condition + (1|SID), data=social_df)
anova(null.model, condition.model)  
#   Res.Df    RSS Df Sum of Sq      F  Pr(>F)  
# 1   5599 100.40                              
# 2   5598 100.35  1  0.048764 2.7202 0.09914 .
summary(null.model) 
# Fixed effects:
#            Estimate Std. Error t value Pr(>|t|)    
# (Intercept) 2.294302   0.001789    1282   <2e-16 ***

#when compare social, no difference 
```
