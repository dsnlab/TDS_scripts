...
throwaway <- lapply(list_of_scales,
                    function(aScale){
                        wide_data_frame <- 
                            scorequaltrics::widen_qualtrics_long(tds2_wave1_scored,
                                                                 scale_names = aScale)
                        filename <- paste0(paste(TDS2_name, 
                                                 wave1_name,
                                                 gsub('\\.', '_', 
                                                      make.names(aScale)),
                                                 sep = '-'),
                                           '.csv')
                        pathname <- file.path(output_file_dir, filename)
                        write.csv(wide_data_frame, file = pathname, row.names = F)
                    })

tds2_wave1_pdss_filename <- paste0(paste(TDS2_name,
                                         wave1_name,
                                         'PDSS',
                                         sep = '-'),
                                   '.csv')

write.csv(scorequaltrics::widen_qualtrics_long(tds2_wave1_scored_pdss,
                                               scale_names = 'PDSS'),
          file = file.path(output_file_dir,
                           tds2_wave1_pdss_filename),
          row.names = F)
```
...
save_scored_scales_wide_csv <- function(dataDF, file_prefix, scale_names = NULL){
    wide_data_frame <- 
        scorequaltrics::widen_qualtrics_long(tds2_wave1_scored,
                                             scale_names = aScale)
    filename <- paste0(paste(TDS2_name, 
...
list_of_scales <- as.list(unique(tds2_wave2_scored_c$scale_name))
...
Get data, rubrics, recode, and clean. 
