...
Instead of converting our data to play nice with the psych package, we can use it to score this questionnaire. 
Doing so gives us quick stats on reliability, too.
...
We could get the pairs plot with the data we scored earlier too using a long-to-wide conversion function.

```{r tds2wave1BIS-15plotpsych}
#get it in wide format so we can use the `psych` package to plot.
