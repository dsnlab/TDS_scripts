...
---
  ...
  # This script identifies events within Cyberball run 2 and pmods
...
labeled_cyberball_folder<-'behavior/CyberBall/multiconds/TDS1_labeled_cyb_data/'
...
ct.info<-ct.info[,c(29, 8,10:11,16:19,27:28)]

#Construct NOD structure for:

#1. Wait
wait.info<-subset(df, behavior.num==2)
wait.info$names<-"wait"
wait.info$onsets<-wait.info$wait.onsets
wait.info$durations<-wait.info$wait.durations
wait.info$pmod<-NA
wait.info<-wait.info[c(2,19:22)]
...
#3. CT_2P
ct_2P.info<-subset(ct.info, behavior.num.x==1)
ct_2P.info$names<-"ct_2P"
ct_2P.info$onsets<-ct_2P.info$ct_2P.onsets
ct_2P.info$durations<-0
ct_2P.info$pmod<-NA
ct_2P.info<-ct_2P.info[,c(1,11:14)]

#4. CT_2C
ct_2C.info<-subset(ct.info, behavior.num.x==0)
ct_2C.info$names<-"ct_2C"
ct_2C.info$onsets<-ct_2C.info$ct_2C.onsets
ct_2C.info$durations<-0
ct_2C.info$pmod<-ct_2C.info$pmod_ct_2C
ct_2C.info<-ct_2C.info[,c(1,11:14)]
...
# Merge all condition types
file<-rbind(wait.info, pt.info, ct_2C.info, ct_2P.info,pc.info)

# Create csv file
 # Change the names from string into numbers:
    # 1: wait
    # 2: pt
    # 3: ct_2P
    # 4: ct_2C
  ...
  file$meanC_pmod = file$pmod - 6.419 #Comment out this line and run script without meanC_pmod first. Use this output with "Finding grand mean.R" script to identify appropriate value for centering. Then re-run this script with correct mean value.
