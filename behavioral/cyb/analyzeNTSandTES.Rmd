...
setwd("/Volumes/research/tds/behavioral/processed/qualtrics/copies")
...
df.nts$SID=df$SID
df.nts=merge(df.nts, subjectList) #subset by 65 subjects in cyb analysis
...
mean_ntsTotal=mean(df.nts$NTS_total_score_score, na.rm=TRUE)/df.nts$NTS_total_score_n_items[1]
sd_ntsTotal=sd(df.nts$NTS_total_score_score,na.rm=TRUE)/df.nts$NTS_total_score_n_items[1]

# individual averages
df.nts.ind=as.data.frame(df.nts$SID)
df.nts.ind$total=df.nts$NTS_mean_score_score
...
```{r}
setwd("/Users/theresacheng/Dropbox (PfeiBer Lab)/TDS/TDS-II/data/Qualtrics/Scored_Qualtrics/TDS2_Master")
