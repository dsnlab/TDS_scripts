...
rx_scripts = {'hpc_F_conj_pmod_flexi'}; % 'F_2x2_age.mat'; do NOT incldude the .mat
...
analysis_folder = '/Volumes/psy-ctn/psy-ctn/TDS/nonbids_data/derivatives/fMRI/rx/cyb/tds2_quad_N69/';  % Make sure that the desired rx directories exist within this analysis folder! Make sure this ends with '/'

fx_folder = '/Volumes/psy-ctn/psy-ctn/TDS/nonbids_data/fMRI/fMRI/analysis/fx/models/cyb/tds2_quad/'; % root folder for fx models, right up to the subject ID, make sure this ends with '/'
...
con_list_conj_excQuad={'con_0007.nii','con_0011.nii', 'con_0015.nii'};
conds_conj_excQuad= [1;2;3];

fileID= fopen('~/projects/dsnlab/tds/TDS_scripts/fMRI/fx/shell/schedule_spm_jobs/cyb/tds2_quad/subject_list_fx.txt') % create file identifier for subject list
...
%% replace paths with CTN server + add excQuad
cd(rx_folder)
load(strcat(char(rx_scripts),'.mat'))
matlabbatch{1, 1}.spm.stats.factorial_design.dir={'/Volumes/psy-ctn/psy-ctn/TDS/nonbids_data/derivatives/fMRI/rx/cyb/tds2_quad_N69/F_conj_pmod_flexi_excQuad'}

% replace existing con and condition fields
for i = 1:length(subject_list)
    for j = 1:length(con_list_conj_excQuad)
        matlabbatch{1,1}.spm.stats.factorial_design.des.fblock.fsuball.fsubject(i).scans{j,1}= ...
        char(strcat(fx_folder,subject_list(i),'/',con_list_conj_excQuad(j)));
        matlabbatch{1,1}.spm.stats.factorial_design.des.fblock.fsuball.fsubject(i).conds=conds_conj_excQuad;
    end
end

save(strcat(rx_folder, 'tds2_quad/F_conj_pmod_flexi_excQuad', 'matlabbatch'))

%% add age
% age_folder = '/Volumtes/psy-ctn/psy-ctn/TDS/behavior/CyberBall/tds2_demographics' 
