...
tds_reduced_mri<- tds_reduced %>% filter(scan > 0)
...
tds_reduced_bad.go <- tds_reduced_mri %>% filter(subject.name != '127') %>% group_by(subject.name) %>% summarize(s_bad.go = sum(n_bad.go, na.rm = TRUE))
